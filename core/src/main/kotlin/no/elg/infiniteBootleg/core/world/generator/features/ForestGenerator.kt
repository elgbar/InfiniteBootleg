...
class ForestGenerator(override val seed: Long, private val cutoff: Double) : FeatureGenerator {
  ...
  private fun createTree(world: World, worldX: WorldCoord, worldY: WorldCoord) {
    ...
    val locs = World.getLocationsAABBFromCorner(worldX.toDouble(), worldY.toDouble() + treeHeight, 0.0, treeHeight.toDouble())
    Material.BirchTrunk.createBlocks(world, locs, prioritize = false, allowOverwriteNonAir = true)
