...
import no.elg.infiniteBootleg.world.world.World.Companion.HALF_BLOCK_SIZE
...
class ForestGenerator(override val seed: Long, private val cutoff: Float) : FeatureGenerator {
  ...
  private fun createTree(world: World, worldX: WorldCoord, worldY: WorldCoord) {
    ...
    if (treeHeight >= MIN_TREE_HEIGHT_FOR_SECONDARY_LEAVES) {
      val leavesRadius2 = shouldGenerateTreeNoise.getNoise(worldX, worldY, 3, MAX_SECONDARY_LEAVES_RADIUS).coerceIn(MIN_SECONDARY_LEAVES_RADIUS, leavesRadius - HALF_BLOCK_SIZE)
