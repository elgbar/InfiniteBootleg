...
class ForestGenerator(override val seed: Long, private val cutoff: Float) : FeatureGenerator {
  ...
  private fun createTree(world: World, worldX: Int, worldY: Int) {
    val treeHeight = (shouldGenerateTreeNoise.getNoise(worldX.toDouble(), worldY.toDouble(), 0.5) * 16)
    ...
    val leavesRadius = (shouldGenerateTreeNoise.getNoise(worldX.toDouble(), worldY.toDouble(), 1.0) * 5)
