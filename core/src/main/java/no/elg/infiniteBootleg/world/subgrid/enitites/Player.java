...

    {
        if (Settings.client) {
            synchronized (LIGHT_LOCK) {
                torchLight = new ConeLight(ClientMain.inst().getSingleplayerWorld().getRender().getRayHandler(), 64, Color.TAN, 48, 5, 5, 0, 30);
...
//        if (getWorld().isClient() && hasControls() && !getVelocity().isZero(0.01f)) {
