...
public final class HelpfulConsoleHelpUtil {
    ...
    private static String[] getArgNames(Method method, Class<?>[] params) {
        ...
        if (annotation != null) {
            ...
            if (names.length != params.length) {
                Main.logger().warn(
                    "Command argument names annotation is present on command '%s', but there are too %s names. " + "Expected %d " + "names " + "found %d",
                    method.getName(), (names.length < params.length) ? "few" : "many", params.length, names.length);
            }
        }
        return names;
    }
     ...
     *     The console used
     * @param exec
     *     The executor to get the methods from, will look at all superclasses
     * @param command
     *     Command method to get, if null no filtering will be done
     *
     * @return All relevant command method
     */
    public static Set<Method> getRelevantMethods(@NotNull Console console, @NotNull CommandExecutor exec, @Nullable String command) {
