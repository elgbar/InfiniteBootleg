...
class TexturedChunkImpl(
  world: World,
  chunkX: ChunkCoord,
  chunkY: ChunkCoord
) : ChunkImpl(world, chunkX, chunkY), TexturedChunk, ViewableChunk, Chunk {
   ...
   * dirty flag of the chunk is set and either [isAllSkyAir] or [texture]
   * called.
   */
  ...
  override suspend fun doUpdateLightMultipleSources0(sources: WorldCompactLocArray, checkDistance: Boolean): Boolean {
    val anyRecalculated = super.doUpdateLightMultipleSources0(sources, checkDistance)
    if (anyRecalculated) {
      queueForRendering(prioritize = false)
    }
    return anyRecalculated
  }
